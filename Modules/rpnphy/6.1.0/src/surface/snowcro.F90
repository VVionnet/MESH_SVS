...
!
 CALL SNOWNLFALL_UPGRID(TPTIME, OGLACIER,                                       &
                        ...
                        PSNOWDIAMOPT,PSNOWSPHERI,PSNOWHIST,PSNOWAGE,GSNOWFALL,GFRZRAIN,   &
...
! except in the case of regridding (SNOWCROEVAPGONE)
DO JST = 1, IMAX_USE
  DO JJ = 1,SIZE(ZSNOW)
    IF (JST <= INLVLS_USE(JJ)) THEN
...
DO JST = 1, IMAX_USE
  DO JJ = 1,SIZE(ZSNOW)
    IF (JST <= INLVLS_USE(JJ)) THEN
      ...
      IF (JST .LT. INLVLS_USE(JJ)) THEN
         PSNOWLIQ  (JJ,JST+1) = PSNOWLIQ  (JJ,JST+1) + MAX(0.,PSNOWLIQ(JJ,JST) - ZWHOLDMAX(JJ,JST))
      ELSE
