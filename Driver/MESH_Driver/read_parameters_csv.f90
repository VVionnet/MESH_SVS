...
subroutine read_parameters_csv(shd, iun, fname, ierr)
    ...
    do while (z == 0)
        ...
        select case (uppercase(args(1)))
            ...
            case (VN_SVS_SNOSPHERIV_SVS)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    if (SHDFILEFMT == 2) then
                        if (shd%lc%NML /= 1) then
                            field1 = adjustl(VN_SVS_SNOSPHERIV_SVS)
                            field2 = adjustl(VN_SVS_SNOSPHERIV_SVS_N)
                            field3 = 'points'
                            icondition = 1
                        end if
                        p = shd%lc%NML
                    ...
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snospheriv_svs, p, b, args(2:), pkey%MAP_ASSIGN_ORDER2, istat)
                    b = svs_mesh%vs%nsl
                end if
            case (VN_SVS_SNOSPHERIV_SVS_N)
                if (.not. svs_mesh%PROCESS_ACTIVE .or.  svs_mesh%vs%schmsol == 'SVS' .or. .not. svs_mesh%vs%lread_restart ) then
                    istat = istat + radix(istat)**pstat%INACTIVE
                else
                    call assign_line_args(k, args(2), istat)
                    ...
                    nargs = nargs - 1
                    b = svs_mesh%vs%nsl
                    call assign_line_args(svs_mesh%vs%snospheriv_svs, p, b, args(3:), pkey%MAP_ASSIGN_ORDER2, istat, k)
                    b = svs_mesh%vs%nsl
                end if
